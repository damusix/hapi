...
import * as Heavy from '@hapi/heavy';
import * as Hoek from '@hapi/hoek';
import * as Mimos from '@hapi/mimos';
import * as Podium from '@hapi/podium';
import * as Statehood from '@hapi/statehood';
...
const Core = class {
    ...
    constructor(options) {
        ...
        this.heavy = new Heavy.Heavy(this.settings.load);
        this.mime = new Mimos.Mimos(this.settings.mime);
    ...
    _initializeCache() {
        ...
        if (!this.caches.has('_default')) {
            this._createCache([{ provider: CatboxMemory.Engine }]);        // Defaults to memory-based
        }
    }
