...
const content = async function (response) {
...
const state = async function (response) {
...
const type = function (response) {

    const _type = response.contentType;
    if (_type !== null && _type !== response.headers['content-type']) {
        response.type(_type);
    }
};


const entity = function (response) {
...
const unmodified = function (response) {
    ...
    const etag = request._core.Response.unmodified(request, _entity);
    if (etag) {
        response.code(304);
...
export { cache, content, state, type, entity, unmodified };
