...
const validator = function (_validator) {

    Hoek.assert(_validator, 'Missing validator');
    Hoek.assert(typeof _validator.compile === 'function', 'Invalid validator compile method');

    return _validator;
};


const compile = function (rule, _validator, realm, core) {

    _validator = _validator ?? internals.validator(realm, core);
    ...
    Hoek.assert(_validator, 'Cannot set uncompiled validation rules without configuring a validator');
    return _validator.compile(rule);
};
...
const params = function (request) {

    return internals.input('params', request);
};


const payload = function (request) {
...
const response = async function (request) {
    ...
    const statusCode = _response.isBoom ? _response.output.statusCode : _response.statusCode;
    ...
    const source = _response.isBoom ? _response.output.payload : _response.source;
    ...
    try {
        ...
        if (value !== undefined &&
            ...
            if (_response.isBoom) {
                _response.output.payload = value;
            }
            else {
                _response.source = value;
            }
        }
    }
    catch (err) {
...
export { validator, compile, headers, params, payload, query, state, response };
