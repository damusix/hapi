...
describe('Core', () => {
    ...
    it('uses a provided listener with manual listen', async () => {
        ...
        const listen = () => {
            ...
            return new Promise((resolve) => listener.listen(0, '0.0.0.0', resolve));
        };

        await listen();
        await server.start();
        const { payload } = await Wreck.get('http://127.0.0.1:' + server.info.port + '/');
        expect(payload.toString()).to.equal('ok');
        await server.stop();
    });
    ...
    describe('_createCache()', () => {
        ...
        it('provisions cache using engine instance', async () => {
            ...
            const engine = new CatboxMemory.Engine();
    ...
    describe('inject()', () => {
        ...
        it('`request.isInjected` access is read-only', async () => {
            ...
            server.route({ method: 'GET', path: '/', handler: (request) => {
                ...
                expect(illegalAssignment).to.throw('Cannot set property isInjected of #<Request> which has only a getter');
