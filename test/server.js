...
describe('Server', () => {
    ...
    describe('cache()', () => {
        ...
        it('provisions a server cache with custom partition', async () => {
            ...
            const server = Hapi.server({ cache: { provider: { constructor: CatboxMemory.Engine, options: { partition: 'hapi-test-other' } } } });
            const cache = server.cache({ segment: 'test', expiresIn: 1000 });
            await server.initialize();
        ...
        it('allows reusing the same cache segment (server)', () => {
            ...
            const server = Hapi.server({ cache: { provider: CatboxMemory.Engine, shared: true } });
            expect(() => {
        ...
        it('emits a cache policy event with named cache provision', async () => {
            ...
            const server = Hapi.server();
            await server.cache.provision({ provider: CatboxMemory.Engine, name: 'named' });
            const cachePolicyEvent = server.events.once('cachePolicy');
    ...
    describe('cache.provision()', () => {
        ...
        it('provisions a server cache (before initialization)', async () => {
            ...
            const server = Hapi.server();
            await server.cache.provision({ provider: CatboxMemory.Engine, name: 'dynamic' });
    ...
    describe('register()', () => {
        ...
        it('sets version to 0.0.0 if missing', async () => {
            ...
            const res = await server.inject('/');
            expect(res.result).to.equal(JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8')).version);
        });
